...
public class TServerPacketLoggerMixin
{
    ...
    public static void process()
    {
        try
        {
            ...
            if(client == null || client.getPacketWriter() == null)
            {
                return;
            }
            ...
            if(packet == null || buffer == null || buffer.getArray() == null)
            {
                return;
            }

            int length = writer.getServerPacketLength();
            int payloadSize = Math.min(Math.max(length, 0), buffer.getArray().length);
            if(payloadSize <= 0)
            {
                return;
            }
            ...
            System.arraycopy(buffer.getArray(), 0, bytes, 0, payloadSize);
            int id = packet.getId();
