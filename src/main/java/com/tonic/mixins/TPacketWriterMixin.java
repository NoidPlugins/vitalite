...
public abstract class TPacketWriterMixin implements TPacketWriter
{
    ...
    @Inject
    private static TClient currentClient()
    {
...
//    }
    ...
    public static void addNodeHook(TPacketBufferNode node)
    {
        try
        {
            ...
            int available = Math.max(0, Math.min(offset, bytes.length) - 1);
            int payloadSize = (len > 0) ? Math.min(len, available) : available;
            if(payloadSize <= 0)
            {
                return;
            }
    ...
    public void clickPacket(int mouseButton, int mouseX, int mouseY)
    {
        ...
        args.put("x", mouseX);
        args.put("y", mouseY);
        this.addNodeSwitch(PacketMapReader.createBuffer(entry, args).toPacketBufferNode(currentClient()));
    }
...
//    }
    ...
    {
        MapEntry entry = PacketMapReader.get("OP_GAME_OBJECT_ACTION_" + (type + 1));
        if(entry == null)
        {
    ...
    {
        MapEntry entry = PacketMapReader.get("OP_GROUND_ITEM_ACTION_" + (type + 1));
        if(entry == null)
        {
    ...
    {
        MapEntry entry = PacketMapReader.get("OP_PLAYER_ACTION_" + (type + 1));
        if(entry == null)
        {
    ...
    {
        MapEntry entry = PacketMapReader.get("OP_NPC_ACTION_" + (type + 1));
        if(entry == null)
        {
