...
public class DisableTransformer
{
    ...
    public static void patch(ClassNode mixin, MethodNode method)
    {
        ...
        boolean hadNoJavaBody = (toHook.access & (Opcodes.ACC_ABSTRACT | Opcodes.ACC_NATIVE)) != 0;
        toHook.access &= ~(Opcodes.ACC_ABSTRACT | Opcodes.ACC_NATIVE);
        if (hadNoJavaBody)
        {
            toHook.instructions.clear();
            toHook.tryCatchBlocks.clear();
            toHook.localVariables.clear();
            toHook.instructions.add(InsnUtil.generateDefaultReturn(toHook));
            return;
        }
